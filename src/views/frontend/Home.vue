<template>
  <div>
    ...
    <main>
      ...
      <!-- 課程折扣區 -->
      <section class="coupon text-center">
      ...
      <section class="FAQ">
        <div class="FAQ__content">
          ...
          <ul class="FAQ__list">
            <li class="FAQ__item" v-for="(item, i) in FAQs" @click="toggleFAQ(i)" :key="i">
              <div class="FAQ__accordian-title">
                <span>
                  <i :class="{ 'FAQ__accordian-title--rotate': showFAQ === i }" class="yo fas fa-plus"></i>
                </span>
                {{ item.question }}
              </div>
            ...
            <form>
              <div class="form__row mb-sm">
                <div class="form__group">
                  <label class="form__label" for="contactName">
                    姓名 *
                  </label>
                  <input id="contactName" class="form__input focus-light" type="text" placeholder="請填入姓名" required />
                </div>
                <div class="form__group">
                  <label class="form__label" for="contactPhone">
                    聯絡電話 *
                  </label>
                  <input id="contactPhone" class="form__input focus-light" type="tel" placeholder="請填入聯絡電話" required />
                </div>
              </div>
              <div class="form__row mb-sm">
                <div class="form__group">
                  <label class="form__label" for="contactMail">
                    電子信箱 *
                  </label>
                  <input id="contactMail" class="form__input focus-light" type="email" placeholder="請填入電子信箱" required />
                </div>
                <div class="form__group">
                  ...
                  <div class="d-flex">
                    <div>
                      <input class="ml-sm form__radio-input" type="radio" name="time" id="morning" checked />
                      <label for="morning" class="form__radio-label" style="padding-left: 10px;">
                    ...
                    <div>
                      ...
                      <label for="afternoon" class="form__radio-label" style="padding-left: 10px;">
                        <span class="form__radio-button"></span>
                        下午
                      </label>
                    </div>
              ...
              <div class="form__row">
                <div class="form__group">
                  <label class="form__label" for="contactRemark">
                    備註
                  </label>
                  <textarea id="contactRemark" class="form__input focus-light" placeholder="請填入備註訊息"></textarea>
                </div>
              </div>
            </form>
          </div>
...
export default {
  ...
  data() {
    return {
      ...
      features: [
        {
          title: '豐富課程種類',
          text: '沒有不愛動的人，只有不合適你的運動！超過數十種豐富課程、彈性上課時段，讓你想動就動不只想瘦更享受。',
          icon: 'fas fa-skiing',
        },
        {
          title: '高級完善設備',
          text: '在最棒的環境做最喜歡的事！寬敞舒適運動空間、潔淨衛浴設備、器材每年定期淘汰保養，給你最完美的運動體驗。',
          icon: 'fas fa-dumbbell',
        },
        {
          title: '全方位專屬規劃',
          text: '專業運動營養顧問一對一諮詢，嚴格管控身體健康數據，讓每滴汗水發揮最大價值，進步改變看得到！',
          icon: 'fas fa-pen-nib',
        },
        {
          title: '堅強師資陣容',
          text: '百位執照教練，豐富經驗生動上課方式，帶你體會動起來的樂趣，這不是在運動，而是在過生活！',
          icon: 'fas fa-users',
        },
      ],
      ...
      FAQs: [
        {
          ...
          answer: '課程內容會因應學生目標和進度而調整，多數人都可進行健身訓練。除非醫生建議您不可進行此類運動，或您未滿18歲需要家長同意。',
        },
        {
          question: '除了訓練費用外，是否有其他雜費要付？',
          answer: '本公司的客戶只須要付私人訓練的課堂費用，無額外的會藉費、入場費、儲物櫃費用等等。',
        },
        {
          question: '參加課程會否有飲食按排或建議？',
          answer: '教練會給予個人化的飲食或補充品建議，並使您慢慢適應飲食改變、因應您的進度作出建議，不會課程剛開始就完全地改變您的飲食餐單。',
        },
        {
          question: '教練擁有專業資格嗎？如何核實？',
          answer: '我們的教練經嚴格考核、定期進修，以確保您享有優質服務。我們提供高透明度資訊，歡迎您隨時上網查閱有關教練專業資格及資歷。',
        },
        {
          question: '客戶個人資料用途？',
          answer: '我們收集的所有客戶資料，包括姓名、電話、電郵、以至訓練情況，都只供本公司對客戶上課使用，絕不會用作傳銷或給予第三方。',
        },
      ],
      showFAQ: 0,
    };
  ...
  computed: {
    ...
    coursesNew() {
      const vm = this;
      return vm.$store.state.courses.courses.filter((course) => course.title.includes('NEW') && course.remainQuantity > 0);
    },
    // 精選課程
    courses() {
      const vm = this;
