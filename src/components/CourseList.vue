<template>
  <div class="courses-list" v-if="coursesList.length > 0">
    <router-link :to="{ name: 'CourseDetails', params: { course_id: course.id } }" class="course-list__course" v-for="course in remainCourse" :key="course.id">
      ...
      <div class="course-list__content">
        <div class="course-list__info">
          <div class="course-list__title heading-tertiary--dark">
          ...
          <div class="course-list__intro d-none-phone">
            {{ textLimit(course.intro, 50) }}
          </div>
        </div>

        <div class="course-list__price">
          <div class="course-list__price-info">
            <div class="course-list__price--discount">
              {{ course.price | currency }}
            </div>
            <div class="course-list__price--origin">
              {{ course.originPrice | currency }}
            </div>

            <star-rating :rating="course.avgRating" :increment="0.1" :star-size="17" :read-only="true" style="margin-left: auto;"></star-rating>
          </div>
